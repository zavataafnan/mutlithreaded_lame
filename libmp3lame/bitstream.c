...
encodeSideInfo2(lame_internal_flags * gfc, int bitsPerFrame)
{
	...
	/* jabarouti: 
	* save main_data_begin data of the overlapped (extra) section 
	* for using in the main part encoding 
	*/
	esv->header[esv->h_ptr].main_data_array = l3_side->main_data_begin;
...
format_bitstream(lame_internal_flags * gfc)
{
	...
	/*
	* check again that the main_data_begin is calculated correctly...
	*/
	if (gfc->next_chunk.finished_encoding_condition)
	{
		int main_data_for_next_frame = gfc->internal_flags_overlapped->sv_enc.header[gfc->next_chunk.counter_next_frame].main_data_array;
		assert(main_data_for_next_frame == l3_side->main_data_begin);
	}
