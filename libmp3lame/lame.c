                            ...
                            int nsamples, unsigned char *mp3buf, const int mp3buf_size)
{
    ...
    while (nsamples > 0) {
        ...
        if (esv->mf_size >= mf_needed) {
			...
			/*
			* in the multi threading status, after finding the solution,
			* set nsamples to zero
			*/
			if (gfc->next_chunk.is_overlapeed_multithreaded)
			{
				if (gfc->next_chunk.find_the_solution)
				{
					nsamples = 0;
				}
			}
	...
	if (gfc->next_chunk.is_overlapeed_multithreaded)
	{
		assert(gfc->next_chunk.find_the_solution);
	}
