...
/* added by jabarouti:*/
/* pass the header information of overlapped section */
int
lame_set_previous_internal_flag(lame_global_flags * gfp, lame_global_flags * overlapped_gfp)
{
	if (is_lame_global_flags_valid(gfp)) {
		if (is_lame_global_flags_valid(overlapped_gfp)) {
			
			gfp->internal_flags->internal_flags_overlapped = overlapped_gfp->internal_flags;
				//return 0;
		}
	}
	return -1;
...
/* consider the amount of main data and overlapped (extra) data numbers*/
int
lame_set_next_chunk_of_data(lame_global_flags * gfp, int main_number, int extra_number)
{
	if (is_lame_global_flags_valid(gfp)) {
			
			gfp->internal_flags->next_chunk.ntotal_frame_number = main_number;
			gfp->internal_flags->next_chunk.nextra_frame_number = extra_number;
			//return 0;
	}
	return -1;
}

int lame_set_multithread_overlapped(lame_global_flags * gfp, int status)
{
	if (is_lame_global_flags_valid(gfp)) {

		gfp->internal_flags->next_chunk.is_overlapeed_multithreaded = status;
		gfp->internal_flags->next_chunk.counter_next_frame = INDEX_TO_ALIGN+2;
		//return 0;
	}
	return -1;
}
...
/* 
* after finishing the encoding in multithread, 
* return the amount of that should be truncated of the next frame*/
int lame_get_multithread_parameters(lame_global_flags * gfp, int * byte_number_to_truncate)
{
	if (is_lame_global_flags_valid(gfp)) {
		...
		if (is_lame_internal_flags_valid(gfc)) 
		{
			...
			EncStateVar_t *const esv = &gfc->internal_flags_overlapped->sv_enc;


			int bitsPerFrame = getframebits(gfp->internal_flags);
			*byte_number_to_truncate =
				(gfc->next_chunk.counter_next_frame - 1) * bitsPerFrame + (bitsPerFrame - 8) -
				esv->header[gfc->next_chunk.counter_next_frame].main_data_array * 8;

			assert(*byte_number_to_truncate % 8 == 0);
			*byte_number_to_truncate /= 8;
		}
	}
	return -1;
}
