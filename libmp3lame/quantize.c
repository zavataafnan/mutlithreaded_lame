...
static int
bin_search_StepSize_formargin_frame(lame_internal_flags * const gfc, gr_info * const cod_info,
	int desired_rate, const int ch, const FLOAT xrpow[576])
{
	int     nBits;
	int     CurrentStep = gfc->sv_qnt.CurrentStep[ch];
...
static void
amp_scalefac_for_one_band(lame_internal_flags * gfc,
	gr_info * const cod_info, FLOAT xrpow[576], int sfb_selected, int up_down)
{
	SessionConfig_t const *const cfg = &gfc->cfg;
	int     j, sfb;
	FLOAT   ifqstep34;
...
static void
set_scalefac_for_all_bands(lame_internal_flags * gfc,
	gr_info * const cod_info, FLOAT * save_scalefac)
{
	SessionConfig_t const *const cfg = &gfc->cfg;
	int status = 0;


	memcpy(cod_info->scalefac, save_scalefac, sizeof(FLOAT) * SFBMAX);
...
/*jabarouti: */
static int
outer_loop_formargin_frame(lame_internal_flags * gfc, gr_info * const cod_info, const FLOAT * const l3_xmin, /* allowed distortion */
	FLOAT xrpow[576], /* coloured magnitudes of spectral */
	const int ch, const int targ_bits)
{                       /* maximum allowed bits */
	SessionConfig_t const *const cfg = &gfc->cfg;
	...
	int nBits = 0;
	int counter_e = 0;
	int just_one_increase_global = 0;

	//just with the binary search the frame should be encoded!!!
	nBits = bin_search_StepSize_formargin_frame(gfc, cod_info, targ_bits, ch, xrpow);

	assert(nBits <= targ_bits);
...
static void
iteration_finish_one_CBR(lame_internal_flags * gfc, int gr, int ch, int status)
{
	SessionConfig_t const *const cfg = &gfc->cfg;
	III_side_info_t *const l3_side = &gfc->l3_side;
	gr_info *const cod_info = &l3_side->tt[gr][ch];
                   ...
                   const FLOAT ms_ener_ratio[2], const III_psy_ratio ratio[2][2])
{
    ...
    gr_info *cod_info;
	int total_bit_compensate = 0;
	int totalbit_computed = 0;
	int totalbit = 0;
	esv->ResvSizeTemp = esv->ResvSize;
    (void) ResvFrameBegin(gfc, &mean_bits);
	int main_data_for_the_next_frame = 0;
	int desired_main_data_of_overlapped = 0;
	int over_bits = 0;
	int counter_for_fail = 0;

	gfc->next_chunk.finished_encoding_condition = 0;
	...
	/* 
	after encoding this frame analyze that if it is can be extended or shrink to fit to the next frame 
	*/
	if (gfc->next_chunk.is_overlapeed_multithreaded)
		/* 
		after calculation of three frames from overlapped section; try to fit the frame to match with the next chunk of data 
		*/
	if (gfc->sv_enc.h_ptr >= gfc->next_chunk.ntotal_frame_number + 3)
	{
		int bitsPerFrame = getframebits(gfc);

		 /*
		 * calculate the main_data_begin of the next frame to compare with the overlapped version
		 */
		 totalbit = 8 * cfg->sideinfo_len;
		int both_should_be_long = 1;

			for (gr = 0; gr < 2; gr++) {
				for (ch = 0; ch < cfg->channels_out; ch++) {
					gr_info const *const gi = &l3_side->tt[gr][ch];
					totalbit += (gi->part2_length + gi->part2_3_length);

					if (gi->block_type == SHORT_TYPE)
					{
						both_should_be_long = 0;
					}
				}
			}
			
			if(totalbit % 8 != 0)
				totalbit = (totalbit / 8 + 1) * 8;

			/* calculate the next main_data_begin for this frame! this frame should change the main_*/
			int temp_main_data_begin = l3_side->main_data_begin;
			main_data_for_the_next_frame = temp_main_data_begin + (bitsPerFrame - totalbit) / 8;


			/* calculate the index of the frame in the overlapped section */
			gfc->next_chunk.counter_next_frame = 
				gfc->sv_enc.h_ptr - gfc->next_chunk.ntotal_frame_number +1 -1 +1;
			
			/* desired main data that the frame should be changed to this main_data_begin to do the alignment*/
			desired_main_data_of_overlapped = 
				gfc->internal_flags_overlapped->sv_enc.header[gfc->next_chunk.counter_next_frame].main_data_array;

			 /* if the calculated main data is larger than the desired main_data the problem is solved
			 * the required bytes can be compensate by the ancillary data!
			 * because the overlapped section is in the beginning of the coding this condition is usually Ok. 
			 * in some case, we should re-encode the frame to satisfy the condition. In the worst case, It should be done. 
			 */

			 if (main_data_for_the_next_frame >= desired_main_data_of_overlapped)
			 {
				 gfc->next_chunk.finished_encoding_condition = 1;
				 gfc->next_chunk.find_the_solution = 1;
			 }
			 else
			 {
				 counter_for_fail++;
				 /*
				 * desired main data is larger than main, wait for the next two frame, 
				 * the next frame may have larger main_data_begin. but for the last try
				 * I also implement the condition that all the frame will not satisfy the
				 * the main_data_begin. In this condition, I will re-encode the frame to get
				 * lower bit rate. 
				 */
				 if (counter_for_fail >= 3)
				 {
					 /*
					 * compute the required number of bytes.
					 */
					 total_bit_compensate = totalbit - (desired_main_data_of_overlapped - main_data_for_the_next_frame) * 8;
					 /*
					 work on the LONG_TYPE frame. both granules should be LONG_TYPE
					 */
					 if ((total_bit_compensate - 8 * cfg->sideinfo_len) > 0 && both_should_be_long == 1)
					 {
						 esv->ResvSize = esv->ResvSizeTemp;
						 (void)ResvFrameBegin(gfc, &mean_bits);

						 /*
						 * finding the desired target bits for each channel of each granule. 
						 * re-encode the frame just satisfy the target bits.
						 */
						 totalbit = total_bit_compensate - 8 * cfg->sideinfo_len;
						 assert(totalbit > 0);
						 /* quantize! */
						 for (gr = 0; gr < cfg->mode_gr; gr++) {
							 for (ch = 0; ch < cfg->channels_out; ch++) {
								 ...
								 if (init_xrpow(gfc, cod_info, xrpow)) {
									  ...
									  */
									 (void)calc_xmin(gfc, &ratio[gr][ch], cod_info, l3_xmin);
									 (void)outer_loop_formargin_frame(gfc, cod_info, l3_xmin, xrpow, ch, targ_bits[ch]);
								 }
						 ...
						 /*
						 * calculate the number of bits after re-encoding and check the condition. 
						 */
						 totalbit_computed = 0;
						 for (gr = 0; gr < 2; gr++) {
							 for (ch = 0; ch < cfg->channels_out; ch++) {
								 gr_info const *const gi = &l3_side->tt[gr][ch];
								 totalbit_computed += (gi->part2_length + gi->part2_3_length);

							 }
						 }

						 if (totalbit_computed % 8 != 0)
							 totalbit = (totalbit_computed / 8 + 1) * 8 + 8 * cfg->sideinfo_len;
						 else
							 totalbit = totalbit_computed + 8 * cfg->sideinfo_len;
						 assert(total_bit_compensate >= totalbit);
					 ...
					 } /* (total_bit_compensate - 8 * cfg->sideinfo_len) > 0 && both_should_be_long == 1 */
				 } /* counter_for_fail >= 2 */
			 } /* else of main_data_for_the_next_frame >= desired_main_data_of_overlapped */
	}

	/* finishing the encoding process as usual for frames
	* but should calculate the number of bits should be allocated in the ancillary data
	*/

	if(!gfc->next_chunk.finished_encoding_condition)
		ResvFrameEnd(gfc, mean_bits);
	else
	{
		ResvFrameEnd_formargin_frame(gfc, mean_bits, (main_data_for_the_next_frame - desired_main_data_of_overlapped) * 8);
	}
